...
# Install vlite from github (here note that we are installing the last commit before I left the lab)
devtools::install_github("vloubiere/vlite@697fad86f7de4b77edac8c550a20f3014a955242")
...
If you prefer to clone it from github, it can be found [here](https://github.com/vloubiere/vlite). However, I will continue updating the GitHub version after leaving the lab, so compatibility issues could appear. To be 100% safe, you should therefore use the last commit from before I left in December 2025 (hash: 697fad86f7de4b77edac8c550a20f3014a955242) using the checkout command:

```bash
git clone https://github.com/vloubiere/vlite.git
cd vlite
git checkout 697fad86f7de4b77edac8c550a20f3014a955242
```
...
- mm10_transcript.rds -> CAGE-TSS to transcript end.
- mm10_promoters.rds -> CAGE-TSS to CAGE-TSS+150bp
- mm10_genebody.rds -> CAGE-TSS+150bp to transcript end.
...
- cluster.id: unique PROseq ID
- EPD_id: Eukaryotic Core Promoter gene ID
- gene_id: ENSEMBLE gene id
- transcript_id: ENSEMBLE transcript id
- coor: Coordinates of the CAGE promoter
- seqnames: chromosome
- start: transcript start (CAGE-TSS for + strand)
- end: transcript end (CAGE-TSS for - strand)
- tissue_specificty: Low to High
- CAGE_tot: CAGE total counts
- CAGE_max: CAGE max count
- PROseq_log2_norm: PROseq log2 normalized counts
- PROseq_act: Is the gene active? (based on PROseq counts)
- motif_cluster: motif-based clusters
- GAGA, BREu, TATA, BREd, XCPE1, XCPE2, Motif1, dTCT, hTCT, BBCABW, hInr, dInr, MTE, bridge, DPE, PB: motif score (only for the motif that lay in relevant positions within the promoter)
- overlaps_CGI: TRUE/FALSE
- CGI_length: Length of the overlapping CGI
- GC_perc: % of GC
- obsExpCpG: Observed versus expected CG ratio
- chrom_cluster: Chromatin cluster
- Pol2_log2enr, DNASe_log2enr, K27Ac_log2enr, K4me3_log2enr, K4me1_log2enr, K119ub1_log2enr, K27me3_log2enr, K9me3_log2enr, methylation_log2enr: enrichment for various PTMs + methyltion
- ORFtag_promoters: promoters used in ORFtag.  
...
The most important script is the last one, where I assemble the actual database as well as the PROseq annotation file. Both files where finally backed in our shared annotations folder:
- PROseq annot (CAGE-TSS -10nt to transcript end) -> `/groups/stark/annotations/mm10/PROseq/EPD_core_promoters_mm10_final_db.rds`
- Database: `/groups/stark/annotations/mm10/PROseq/EPD_core_promoters_mm10_final_db.rds`
