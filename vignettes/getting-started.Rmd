...
<p>The <code>vlite</code> package was designed for exploratory analyses of genomic datasets, and focuses on simplicity and clarity. It contains a set of helper functions as well as wrappers/pipelines:</p>
...
-   **Loading** the package is quick and especially useful during development, when functions are being frequently updated. It ensures you're always working with the most recent version. However, you'll need to reload the package every time you restart your R session. Here is how to do it:
...
# Install vlite from my folder
devtools::install("/groups/stark/vloubiere/vlite/")

# Install vlite from github (here note that we are installing the 'stark_2025' branch I created when I left the lab)
devtools::install_github("vloubiere/vlite@stark_2025")
...
# III- Modify the package functions

Sooner or later, you may want to modify a function in this package. The easiest way is to use a standard Git workflow:\
- Copy the package folder locally (recommended) or clone the repo from GitHub (see below).\
- Make your changes.\
- Load your modified version in R: `devtools:load_all("/my/modified/package/directory/")`.

You’ll now be using your updated functions! If you want to do more complex changes (add functions, update documentation...), see this useful [link](https://devtools.r-lib.org/) on how to use devtools. If this sounds like very dark magic to you, ask any bioinfo in the lab or chat-GPT for help.

If you decide to copy the package folder locally, you can copy it from my folder `"/groups/stark/vloubiere/vlite/"`.

If you prefer to clone it from github, it can be found [here](https://github.com/vloubiere/vlite). However, I will continue updating the GitHub version after leaving the lab, so compatibility issues could appear. To be 100% safe, you should therefore use the 'stark_2025' branch I created when I leaved the lab, using the checkout command:

``` bash
git clone https://github.com/vloubiere/vlite.git
cd vlite
git checkout stark_2025
```

# IV- Before using pipelines

Some pipelines or post-processing functions described below run R code. For those, you must provide the path to the R executable that has all required packages installed (e.g., see the last argument of ?cmd_confidentPeaks function). For this purpose, Matus Vojtek made a conda environment containing all needed packages and a recent R version. It is stored in our shared folder `"/groups/stark/conda_envs/"` and will get automatically loaded when we using the scripts below (see `?vl_submit` for more details).

However, in the future, some new functions or updates might require new packages. In this case, you should update the conda environment accordingly. If you need help with this, ask any bioinfo in the lab or ask chat-GPT.

Alternatively, you can always redirect these functions to your own R install in which all install packages are installed. To install packages, the best it to first open a new terminal (**!! do not use the integrated terminal from Rstudio !!**) on your computer and ssh to the server:

``` bash
ssh user.name@cbe.vbc.ac.at
```

Start the R you plan to use. For example, I sometimes used '/software/f2022/software/r/4.3.0-foss-2022b/bin/R', but you can use a different one if you prefer:
...
You can now repeat the operation for all your samples, either by repeating the previous operation by hand or by using a programmatic loop, like in this example. Note that here, I recommend setting the `output.prefix` parameter to NULL, which will ensure that the output file names are unique. This is very important because, if several input file/BC combinations are given the same output.prefix (re-sequencing, sample split on two lanes...), they will overwrite each other:

```{r loop_example_ORFtag, eval = FALSE}
# For each line in the metadata file (see the `by=` argument on the last line)
meta.demultiplex <- meta[, {
  ...
  cmd <- vlite::cmd_demultiplexVBCfile(
    ...
    output.prefix= NULL, # Here, I recommend setting this to NULL, which will automatically create a unique output file name for each input file / BC combination(s).
    fq.output.folder= "db/fq/" # The output folder where the output fastq files will be saved
  )
  ...
  vlite::vl_submit(
    ...
    logs = "db/logs/demultiplex/", # Chose the folder where log files will be saved
    job.name= sampleID[1] # Job name
  )
  # Return the output files paths
  transpose(cmd[, .(file.type, path)], make.names = T)
}, by= names(meta)] # By specifying all column names here, each line will be processed separately
...
# Check help of the funciton to import fastq files
?vlite::importFq
...
Upon large-scale transduction of cultured cells, inserted ORFtag cassettes will get transcribed, and the user-defined tag (TetR) gets spliced into the closest downstream non-first exon. Thus, to assign insertions to genes, we will need to generate a gtf file that contains the genomic coordinates of all non-first exons.\
I already made these annotation files for mm10 and hg38, using the script below, and backed them up in our shared annotations folder here:\
`/groups/stark/annotations/mm10/ORFtag`\
`/groups/stark/annotations/hg38/ORFtag`

```{r ORFtag annotations, eval= FALSE}
...
# Process each user/batch/screen/condition/replicate/sampleID/genome combination separately (see the `by` arg on the last line)
meta.processed <- meta.demultiplex[, {
  ...
  cmd <- vlite::orftagProcessing(
    ...
    gtf = NULL, # Since we specified mm10, the corresponding gtf will be used (see previous paragraph)
    ...
    Rpath = "Rscript", # The path to the R executable that will be used to run subscripts
  ...
  transpose(cmd[, .(path= paste0(path, collapse= ",")), file.type], make.names = T)
}, by= .(user, batch, screen, condition, replicate, sampleID, genome)] # Each group of the specified columns will be processed separately.
...
cmd <- vlite::cmd_demultiplexVBCfile(
  ...
  layout = "PAIRED",
  i7 = "ACAGTG", # i7 barcode (or RPI in the PRO-seq terminology)
  i5 = "none", # Here we have no i5
...
meta.demultiplex <- meta[, {
  cmd <- vlite::cmd_demultiplexVBCfile(
    vbcFile = bam_path,
    layout = layout,
    i7 = barcode, # i7 barcode (or RPI in the PRO-seq terminology)
    i7.column = 14, # This specifies in which column of the bam file the i7 sequences are stored. By default it is set to 14 but should sometimes be set to 12, depending on the sequencer. If you are not sure in which column are your indexes, check the head of your bam file using samtools view.
    output.prefix= NULL, # Here, I recommend setting this to NULL, which will automatically create a unique output file name for each input file / BC combination(s).
    fq.output.folder= "db/fq/", # The output folder where the output fastq files will be saved
...
# Process each combination of the columns specified in `by` (see last line) separately
meta.processed <- meta.demultiplex[, {
  # Create the command
  cmd <- vlite::proseqProcessing(
    fq1= fq1, # Only the first read is used. Note that several files can be specified (re-sequencing...)
  ...
  # Return commands and output files
  transpose(cmd[, .(path= paste0(path, collapse= ",")), file.type], make.names = T)
}, by= .(user, batch, experiment, cell_line, treatment, replicate, sampleID,
         DESeq2_name, DESeq2_condition, DESeq2_control,
         genome, spikein_genome)] # Each group of the specified columns will be processed separately.
...
For PROseq, we use CAGE-defined TSSs annotations to quantify the reads at TSSs, transcript and gene bodies. I already made these annotation files for mm10 using the script below (adapted from Vanja Haberle), and backed them up in our shared annotations folder here:\
`/groups/stark/annotations/mm10/PROseq/`\
The files of interest are: - mm10_transcript.rds -\> CAGE-TSS to transcript end. - mm10_promoters.rds -\> CAGE-TSS to CAGE-TSS+150bp - mm10_genebody.rds -\> CAGE-TSS+150bp to transcript end.

```{r PROseq annotation files, eval= FALSE}
...
I started to develop a new annotation file, whose purpose is to be coupled to a clear and transparent extensive database, which contains a bunch of information for each promoter:\
- cluster.id: unique PROseq ID - EPD_id: Eukaryotic Core Promoter gene ID - gene_id: ENSEMBLE gene id - transcript_id: ENSEMBLE transcript id - coor: Coordinates of the CAGE promoter - seqnames: chromosome - start: transcript start (CAGE-TSS for + strand) - end: transcript end (CAGE-TSS for - strand) - tissue_specificty: Low to High - CAGE_tot: CAGE total counts - CAGE_max: CAGE max count - PROseq_log2_norm: PROseq log2 normalized counts - PROseq_act: Is the gene active? (based on PROseq counts) - motif_cluster: motif-based clusters - GAGA, BREu, TATA, BREd, XCPE1, XCPE2, Motif1, dTCT, hTCT, BBCABW, hInr, dInr, MTE, bridge, DPE, PB: motif score (only for the motif that lay in relevant positions within the promoter) - overlaps_CGI: TRUE/FALSE - CGI_length: Length of the overlapping CGI - GC_perc: % of GC - obsExpCpG: Observed versus expected CG ratio - chrom_cluster: Chromatin cluster - Pol2_log2enr, DNASe_log2enr, K27Ac_log2enr, K4me3_log2enr, K4me1_log2enr, K119ub1_log2enr, K27me3_log2enr, K9me3_log2enr, methylation_log2enr: enrichment for various PTMs + methyltion - ORFtag_promoters: promoters used in ORFtag.

To assemble this table, I used data from different promoter databases, including the [Eukaryotic Promoter Database](https://epd.expasy.org/epd/mouse/mouse_database.php?db=mouse) and the [Element core database](https://www.juven-gershonlab.org/resources/core-2023/). These data were then processed and compiled using the scripts listed below (and linked: `file.edit("/groups/stark/vloubiere/projects/ORFTRAP_PcG/git_ORFtag_PcG/main.R")`), which you can read to precisely understand how each column was computed:

```{r PROseq annot EPD, eval= FALSE}
# Set working directory
setwd("/groups/stark/vloubiere/projects/ORFTRAP_PcG/")

# Download public data ----
# CpG islands (UCSC)
file.edit("git_ORFtag_PcG/subscripts/UCSC_CGI_to_bed.R")
# EPD promoters (from the database)
file.edit("git_ORFtag_PcG/subscripts/Download_EPD_promoters_mm10.R")
# Corresponding gene annotation (from UCSC)
file.edit("git_ORFtag_PcG/subscripts/Download_EPD_knownGenes_mm10.R")
# Mouse housekeeping genes
file.edit("git_ORFtag_PcG/subscripts/Download_housekeeping_genes_list.R")
# FANTOM5 max counts CAGE
file.edit("git_ORFtag_PcG/subscripts/FANTOM5_CAGE_max.R")

# Make promoter database ----
file.edit("git_ORFtag_PcG/subscripts/EPD_CP_PROseq_activity.R")
file.edit("git_ORFtag_PcG/subscripts/EPD_CP_CGI_obs_vs_exp.R")
file.edit("git_ORFtag_PcG/subscripts/EPD_CP_ELEMENT_motifs_clustering.R")
file.edit("git_ORFtag_PcG/subscripts/EPD_CP_compute_ChIP_DNAse_meth_signal.R")
file.edit("git_ORFtag_PcG/subscripts/EPD_CP_define_chromatin_clusters.R")
file.edit("git_ORFtag_PcG/subscripts/Mm_EPDnew_make_database.R")
```

The most important script is the last one, where I assemble the actual database as well as the PROseq annotation file. Both files where finally backed in our shared annotations folder: - PROseq annot (CAGE-TSS -10nt to transcript end) -\> `/groups/stark/annotations/mm10/PROseq/EPD_core_promoters_mm10_final_db.rds` - Database: `/groups/stark/annotations/mm10/PROseq/EPD_core_promoters_mm10_final_db.rds`

### c- Blacklisted regions

Another thing with PRO-seq is that PolI and PolIII transcripts will also be retrieved. To remove them, I made a list of blacklisted regions using the following script `file.edit("/groups/stark/vloubiere/vlite/inst/Rscript/create_PROseq_blacklisted_regions.R")`:

```{r blacklist PROseq, eval= FALSE}
setwd("/groups/stark/vloubiere/genomes/Mus_musculus/PROseq/")
...
It includes most non-coding PolI/PolIII RNAs (rRNAs, tRNAs, Mt_rRNA, Mt_tRNA, snRNA, snoRNA, scaRNA, miRNA, scRNA, sRNA, misc_RNA, ribozyme, UCSC blacklisted regions) and blacklisted regions (low mappability...).

## 6- Count reads

Now, we will used the annotation files described above to count the number UMI collapsed reads associated to each gene promoter, transcript or gene body. But before this, we ideally want to used the annotated blacklisted regions to remove reads from PolI/PolIIItranscripts. In this example, I will show how to do the counting for full-length transcripts, and you can later repeat the operation for promoters or gene bodies if needed:
...
# At this point, each sample should correspond to a unique entry in the metadata
stopifnot(uniqueN(meta.processed$sampleID)==nrow(meta.processed))
...
meta.counts <- meta.processed[, {
  ...
  cmd <- vlite::cmd_countPROseqReads(
    ...
    blacklist.file = "/groups/stark/vloubiere/genomes/Mus_musculus/PROseq/mm10_blacklisted_regions.rds", # Blacklisted regions
    ...
    Rpath = "Rscript" # The R executable used to run the R script
  )
  # Submit to the cluster
  vl_submit(
    cmd,
  ...
  # Return commands and output files
  transpose(cmd[, .(file.type, path)], make.names = T)
}, by= names(meta.processed)] # By specifying all column names here, each line will be processed separately (one sampleID/line)

# Save the metadata containing count files
saveRDS(meta.counts, "Rdata/count_files.rds")
```

First you should check the fraction of blacklisted reads that were removed, and we typically expect values around 15-20% for mm10: `fread("db/count_tables/AID.Hcfc1_IAA_0h_Hcfc1.depl.N_rep1_mm10_UMI_counts_transcript_counts__blacklisted.txt")`. Here, 116,251 read clusters (or 'peaks') were removed, cumulating 1,957,339 reads (18.4% of the total). Of note, if you are using spike-in, removing these reads will not affect the DESeq2 normalization below; which will therefore only be based on the total number of spike-in Drosophila reads in each sample. But if you chose library size normalization, it will:

```{r removed_counts, echo=FALSE, out.width="60%", fig.align="center", fig.cap= "Number of removed reads."}
knitr::include_graphics(system.file("images/removed_counts.png", package = "vlite"))
```

Let's now look one of the resulting count files (for example using `fread("db/count_tables/AID.Hcfc1_IAA_0h_Hcfc1.depl.N_rep1_mm10_UMI_counts_transcript_counts.txt")`). The first ID column contains the UCSC gene id followed by the gene name and the coordinates of the corresponding CAGE promoter, separated by '\_\_'. So the Xkr4 transcript has 41 UMI-collapsed associated reads in this condition:
...
# Process each experiment separately (see the `by=` argument on the last line)
meta.FC <- meta.counts[, {
  ...
  cmd <- vlite::cmd_DESeq2_PROseq(
    ...
    Rpath = "Rscript" # The R path executable that will be used to run the Rscript.
  )
  # Submit to the cluster
  vl_submit(
    cmd,
  ...
  data.table::transpose(cmd[, .(file.type, path)], make.names = T)
}, experiment] # For each experiment (one DESeq2 object/experiment)

```

## 8- Output files

The above command generates 3 types of files: - The usual DESeq2 FC tables for each DESeq2_condition/DESeq2_control combination specified in the corresponding columns of the metadata. Thus, there is only one here: IAA_3h vs. control (see previous section). - For each FC table, the corresponding MA plot.\
...
In the case of this 3h HCFC1 depletion, we have 3,188 significantly down genes, 55 significantly up and 12,795 unaffected genes (see the next section about MA plot):
...
### c- The DESeq2 object {#c--the-deseq2-object}
...
mat <- SummarizedExperiment::assay(vst_mat)
...
# For each sampleID/genome combination (see the `by=` argument on the last line)
meta.processed <- meta.demultiplex[, {
  # Create the command
  ...
  # Return commands and output files
  transpose(cmd[, .(path= paste0(path, collapse= ",")), file.type], make.names = T)
}, by= .(sampleID, genome)] # Each group of the specified columns will be processed separately.
...
cmd <- vlite::cmd_peakCalling(
  ...
  Rpath = "Rscript" # Path to the R executable used to execute the Rscript.
)
...
- The .narrowPeak file containing the peaks (52,054 peaks called here) - A .xls excel file containing the job parameters and the peaks - A \_treat_pileup.bdg file that contains the reads pile up for the sample (here the V5.HCFC1) - A \_control_lambda.bdg file that contains the local background computed by MACS2.
...
cmd <- vlite::cmd_cutnrunQC(
  ...
  Rpath = "Rscript" # Path to the R executable setup in section III
)
...
1- Make the dictionary index 2- Use the pipeline.
...
```{r simplify masterORF, eval= FALSE}
setwd("/groups/stark/vloubiere/projects/ORFscreen_tomas/")
...
# Import data ----
dat <- readRDS("Rdata/Master_eORFeome_clean_allORFs.rds")
dat <- dat[!is.na(DNAseq)]

# Order the table so that the longest sequences appear on top and will be favored during selection ----
dat[, seqLength:= nchar(DNAseq)]
setorderv(dat,
          c("seqLength", "id"),
          c(-1 ,1))
dat[, idx:= .I]

# Extract all 20-nt k-mers with their sequence index ----
k <- 20
kmers <- dat[, {
  .c <- DNAseq[1]
  if(seqLength<k) stop("Sequence is too short for kmer size")
  .(kmer= sapply(seq(20, seqLength), function(i) substr(.c, i-19, i)))
}, .(idx, id, Pool, seqLength)]
# Count duplicated Kmers
kmers <- kmers[, .(multKmer= .N), .(idx, id, Pool, seqLength, kmer)]

# For each k-mer, get all pairs of sequences sharing it ----
setkeyv(kmers, "kmer")
pairs <- kmers[kmers, allow.cartesian = TRUE]

# When a shorter sequence (higher index) shares kmers with a longer sequence (lower index)
# within the same pool -> count number of Kmers in common
uniqPairs <- pairs[Pool==i.Pool & idx>i.idx, .(Nkmer= sum(multKmer)), .(id = id, sim.id = i.id, seqLength)]

# Find duplicated sequences with <= 50 nt that differ from the most similar yet longer sequence ----
uniqPairs[, Nmismatches:= (seqLength-19)-Nkmer]
dup <- uniqPairs[Nmismatches<=50]

# Initiate the final table with ORFs showing no similarities ----
uniq <- dat[!(id %in% dup$id)]

# Add an extra column containing the ids of similar sequences ----
uniq$similar.id <- dup[uniq, .(.(unique(id))), .EACHI, on= "sim.id==id"]$V1

# Summary ----
total <- length(unique(dat$id))
unambiguous <- length(unique(uniq$id))
ambiguous <- length(unique(na.omit(unlist(uniq$similar.id))))
print(paste(total, "ids in total"))
print(paste(unambiguous, "are unambiguous"))
print(paste(ambiguous, "are ambiguous"))
print(paste("For a total of", unambiguous+ambiguous))

# Clean and save ----
uniq$idx <- uniq$seqLength <- NULL
setcolorder(uniq, c("id", "similar.id"))
setorderv(uniq, "id")
fwrite(uniq,
       "Rdata/tables/collapsed_Master_eORFeome_clean_allORFs.txt",
       sep = "\t",
       quote= F,
       na= NA,
       col.names = T,
       row.names = F)
saveRDS(uniq, 
        "Rdata/collapsed_Master_eORFeome_clean_allORFs.rds")
```
...
```{r make dictionary, eval= FALSE}
# Make dictionary ----
  file.edit("git_ORFscreen/subscripts/extract_VBC_bam_dictionary.R")
  file.edit("git_ORFscreen/subscripts/make_ORF_bowtie2_index.R")
  file.edit("git_ORFscreen/subscripts/trim_BC_ORF_reads.R")
  file.edit("git_ORFscreen/subscripts/align_ORFs.R")
  file.edit("git_ORFscreen/subscripts/make_dictionary_indexes.R")
```

However, the three last scripts (trim, align, make) are old/deprecated and should not be used. Instead, use the recent version I wrote to analyse Paul's new dictionary (gaidt lab):

```{r make dictionary Paul, eval= FALSE}
# Make dictionary Paul ----
  file.edit("git_ORFscreen/subscripts/trim_BC_ORF_reads_paul.R")
  file.edit("git_ORFscreen/subscripts/align_ORFs_paul.R")
  file.edit("git_ORFscreen/subscripts/make_dictionary_indexes_paul.R")
```

However here, note that Paul already had demultiplexed fastq files and this step was skiped.

#### 1- Demultiplex fastq files

The first script `file.edit("git_ORFscreen/subscripts/extract_VBC_bam_dictionary.R")` uses the ?cmd_demultiplexVBCfile function to demultiplex fastq files from VBC NGS facility bam files. If you want other examples on how to use this function, see the 'demultiplexing' sections of any of the pipeline sections above.

#### 2- Make bowtie2 indexes

The second file `file.edit("git_ORFscreen/subscripts/make_ORF_bowtie2_index.R")` uses the simplified/collapsed ORF sequences defined in [2- Simplify the master ORF](##-2--simplify-the-master-ORF) to create a bowtie2 index, which will be used to align the ORF part of the dictionary reads.
...
We therefore use different trimming strategies to extract the two parts we are interested in: the ORF and the 30nt BC, as specified in `file.edit("git_ORFscreen/subscripts/trim_BC_ORF_reads.R")`:\
- For the BC part, we used cutadapt to trim illF and illR sequences from read2, and only retain the sequences that are at least 25nt long. - For the ORF part, we have 3 strategies:\
1- Trim AttB1 and 2 from read 1. 2- Remove the first 30nt of read 1. 3- Remove the last 30nt of read 2.
...
- For each read, only the ORF alignment with the highest mapping quality (MAPQ) were kept, and we required a minimum value of 30. This indicates that the aligner is very confident about the correctness of the alignment (error 1e-03). Therefore, it is almost impossible to get two different ORFs with MAPQ\>=30. - Then, BC/ORF pairs were combined, and all the BC sequences that were associated to \> 1 ORF sequence were discarded (ambiguous). - Finally, only the BC/ORF pairs with at least 3 supporting reads were considered.
...
# Process each screen/condition/cell_line/replicate/sampleID/MAGeCK_sort/dictionary/i7/layout/tar_path combination separately (see the `by` arg on the last line)
meta.demultiplex <- meta[, {
  # Generate command
  cmd <- vlite::cmd_demultiplexVBCfile(
...
# Process each sampleID separately (see the `by` arg on the last line)
meta.processed <- meta.demultiplex[, {
  # Create the command
  ...
  # Return commands and output files
  transpose(cmd[, .(path= paste0(path, collapse= ",")), file.type], make.names = T)
}, by= .(sampleID)] # Each group of the specified columns will be processed separately.
...
- rank: The rank of the ORF based on it's MAGeCK score. - score: MAGeCK's Robust Rank Aggregation score of the enrichment of this ORF in the selected population. - num: The total number of unique barcodes (BCs) detected for this ORF across all samples. - goodsgrna: The number of BCs for this ORF showing significant enrichment in the selected population. - hit: indicate which ORFs are bona fide hits, meaning they satisfy the cutoffs defined in function call (lfc \>= log2(1.5) & FDR\<=0.05). In this case, we retrieved 24 bona fide hits (`table(FC$hit)`).

The table also contains all the additional columns present in the master.table.rds:\
- similar.id: ids of highly similar ORFs. - Gene: gene name...
