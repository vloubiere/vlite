#' Process STAP-Seq Sequencing Data
#'
#' @description
#' This a quick and dirty wrapper around the PRO-Seq pipeline, which does not flip the orientation of the read (like PRO-Seq does).
#'
...
#'
#' @export
stapseqProcessing <- function(fq1,
                             output.prefix,
                             ref.genome,
                             ref.genome.idx= NULL,
                             spike.genome,
                             ...
                             time= '2-00:00:00')
{
  ...
  # Return ----
  cmd$cores <- cores
  cmd$job.name <- paste0("STAP_", output.prefix)
  cmd$mem <- mem
  cmd$time <- time
  return(cmd)
}
