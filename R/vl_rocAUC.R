#' ROC AUC
#'
#' @param ranking_score ranking_score values from the model.
#' @param labels A vector of logical labels (or that can be coerced to logical).
#' @param NES Should the Normalized Enrichment Score be computed (Niter= 1000)? Default= TRUE.
#' @param plot Should the ROC be plotted? Default= FALSE.
#' @param main Main title Default= "ROC".
#' @param xlab xlab. Default= "FPR".
#' @param ylab ylab. Default= "TPR".
#' @param type The type of plot. Default= "l".
#' @param add Should the ROC line be added to an existing plot?
#' @param ... Extra arguments to be passed to plot (when plot= TRUE) or lines (when plot= FALSE and add= TRUE).
#'
#' @return ROC AUC
#' @export
vl_rocAUC <- function(ranking_score,
                      labels,
                      compute.NES= TRUE,
                      plot= FALSE,
                      main= "ROC",
                      xlab= "FPR",
                      ylab= "TPR",
                      type= "l",
                      add= FALSE,
                      ...)
{
  # Checks ----
  if(!is.logical(labels)) {
    labels <- as.logical(labels)
    warning("labels were converted to logical")
  }
  if(plot && add)
    add <- FALSE

  # Compute N pos and neg ----
  n_pos <- sum(labels)
  n_neg <- sum(!labels)
  stopifnot(n_pos > 0, n_neg > 0)

  # Order ----
  labels <-  labels[order(ranking_score, decreasing= T)]

  # Compute TRUE and FALSE positive rate ----
  TPR <- c(0, cumsum(labels) / n_pos)
  FPR <- c(0, cumsum(!labels) / n_neg)

  # AUC via trapezoidal rule ----
  auc <- sum(diff(FPR) * (head(TPR, -1) + tail(TPR, -1)) / 2)

  # bg for NES ---
  if(compute.NES) {
    rdm <- sapply(seq(100), function(i) {
      set.seed(i)
      labels <- sample(labels)
      TPR <- c(0, cumsum(labels) / n_pos)
      FPR <- c(0, cumsum(!labels) / n_neg)
      sum(diff(FPR) * (head(TPR, -1) + tail(TPR, -1)) / 2)
    })
    NES <- (auc - mean(rdm)) / sd(rdm)
  }

  # Plot ----
  if(add)
  {
    lines(FPR,
          TPR,
          ...)
  }else if(plot)
  {
    plot(FPR,
         TPR,
         xlab= xlab,
         ylab= ylab,
         type= type,
         main= main,
         ...)
    abline(0, 1, lty = 2, col = "grey")
  }

  # Return
  var <- c(auc= auc)
  if(compute.NES)
    var <- c(var, c(NES= NES))
  return(var)
}
